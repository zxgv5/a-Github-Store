...
- **`X-GitHub-Token` header:** Forwarded to the backend on every route that does a live GitHub passthrough — `/v1/search`, `/v1/search/explore`, `/v1/repo/{owner}/{name}` (only on the lazy-fetch DB-miss path), `/v1/releases/{owner}/{name}`, `/v1/readme/{owner}/{name}`, and `/v1/user/{username}`. The backend re-sends it as `Authorization: token $token` to `api.github.com`, so the upstream call runs under the user's own 5000/hr OAuth quota instead of the 60/hr-per-IP anonymous bucket. Without it, a popular repo's releases endpoint can poison the backend's 15-min negative cache (`negativeTtlSeconds = 900`, key `releases:{owner}/{name}?page=…&per_page=…` is shared across users) on a single quota burst. DB-only routes never see the header: `/v1/categories`, `/v1/topics`, `/v1/events`, `/v1/auth/device/start`, `/v1/auth/device/poll`, `/v1/badge/...`. Sourced from `TokenStore.currentToken()` via `BackendApiClient.currentUserGithubToken()` (`private`); never logged (no Ktor `Logging` plugin installed). **401 from passthrough routes** (`/v1/repo`, `/v1/releases`, `/v1/readme`) means GitHub upstream rejected the forwarded token, *not* "your backend session is over" — backend reserves its own 401 semantics for routes that don't currently exist. The client's `UnauthorizedInterceptor` only installs on `createGitHubHttpClient` (direct GitHub calls), so backend 401s never trip session-expired directly; `AuthenticationStateImpl` additionally debounces consecutive 401s under the same token so a single transient hit can't sign the user out.
